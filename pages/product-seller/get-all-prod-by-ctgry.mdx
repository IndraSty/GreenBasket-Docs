...
Fetch all data product by category
...
GET /api/sellers/current/stores/{store_id}/products/category
```
...
`key` : string (required) -> `key` is the `category` name
...
{
  "message": "Fetch All product seller by category Successfully",
  "result": {
    "products": [
      {
...
If query to `database` get troble while get the `store` data by `id`

```json filename="Response Body"
{
  "error": "failed to get store by id"
}
```